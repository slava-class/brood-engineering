	        ...
	        on_tick(function()
	            ...
	            if phase == "waiting_assigned" then
	                ...
	                if phase_age() > constants.main_loop_interval * 30 then
	                    ...
	                    error(("Timed out waiting for assignment (tick=%d, status=%s, task=%s)")
	                        :format(tick, status, task_id))
	                end
	            ...
	            elseif phase == "waiting_completed" then
	                ...
	                if phase_age() > 60 * 12 then
	                    ...
	                    error(("Timed out waiting for completion (tick=%d, status=%s, task=%s, speed=%.3f, dests=%d, dist=%.2f)")
	                        :format(tick, status, task_id, speed, dest_count, dist))
	                end
	            ...
	            elseif phase == "waiting_recall" then
	                ...
	                if idle_since_tick and
	                   tick - idle_since_tick >
	                   (constants.no_work_recall_timeout_ticks + constants.main_loop_interval * 10) then
